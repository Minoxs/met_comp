...
use Metodos_Computacionais_Fisica, only: dp, zero, half, one, two, &
                                         four, ten, z0, fzabst 
implicit none
! Variaveis mudas.
...
real(dp), intent(in)                              :: errabs, errrel
real(dp), optional, intent(in)                    :: fator
complex(dp), dimension(nnov+nprev), intent(inout) :: czeros
procedure(fzabst)                                 :: fn
! Variaveis locais.
logical           :: tes_maxit, tes_g_mxit, tes_real, tes_iter
integer           :: ntot, i, n_it
real(dp)    :: eps1, eps2, teste, h_ini= 0.1_dp, tiny1= tiny(one)
real(dp), parameter    :: big1= huge(one)
complex(dp), parameter :: z1o10=(0.1_dp,zero), z2=(two,zero), z4=(four,zero)
complex(dp) :: c, den, divdf1, divdf2, dvdf1p, fzr, z_inc, zrprev
complex(dp) :: fzrdfl, fzrprv, h, hprev, czero, sqr, him2, him1
if(nnov < 1)then
...
CONTAINS
   ...
   complex(dp), intent(out)   :: fzero, fzrdfl
   complex(dp), intent(inout) :: z_inc
   logical           :: t_den= .true.
   integer           :: j
   real(dp), dimension(2) :: v_sft
   complex(dp) :: root, den
 !
   l_den: do
      if (n_it == maxit)then
...
! Desloca o ponto aleatoriamente.
            ...
            z_inc= z_inc + cmplx(v_sft(1), v_sft(2), dp)
            t_den= .false.
            exit l_deflac
         else
            fzrdfl = fzrdfl/den
